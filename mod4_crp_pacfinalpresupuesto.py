...
NOMBRE_SALIDA = "RESULTADO_CRP_PAC.xlsx"

# ============================================================
# DETECTAR ARCHIVOS DE ENTRADA
# ============================================================
xlsx_files = []
carpetas = [BASE, os.path.dirname(BASE)]
for carpeta in carpetas:
    try:
        for f in os.listdir(carpeta):
            ruta_f = os.path.join(carpeta, f)
            if (f.lower().endswith(".xlsx") and not f.startswith("~$")
                    and not f.startswith("_") and f != NOMBRE_SALIDA
                    and ruta_f not in xlsx_files):
                xlsx_files.append(ruta_f)
    except:
        pass

print(f"Archivos xlsx encontrados: {[os.path.basename(x) for x in xlsx_files]}")
...
for ruta in xlsx_files:
    ...
    try:
        df_test = pd.read_excel(ruta, sheet_name=0, nrows=5, dtype=str)
        cols = [str(c).strip() for c in df_test.columns]
        if 'Tipo Registro P' in cols and 'Clave Contab.' in cols:
            ruta_pagos = ruta
            print(f"Plantilla Pagos detectada: {nombre}")
    except:
        pass
    try:
        xl = pd.ExcelFile(ruta)
        if 'Data' in xl.sheet_names:
            df_test = pd.read_excel(ruta, sheet_name='Data', nrows=5, dtype=str)
            cols = [str(c).strip() for c in df_test.columns]
            if 'Disponibilidad PAC' in cols or 'Per.presup.' in cols:
                ruta_pac = ruta
                print(f"Reporte PAC detectado: {nombre}")
    except:
        pass
    try:
        df_test = pd.read_excel(ruta, sheet_name=0, nrows=5, dtype=str)
        cols_lower = [str(c).strip().lower() for c in df_test.columns]
        if any('crp' in c for c in cols_lower) and any('rubro' in c for c in cols_lower) and ruta != ruta_pagos:
            ruta_crp = ruta
            print(f"Reporte CRP detectado: {nombre}")
    except:
        pass

if not ruta_pagos:
    print("Error: No se encontro la Plantilla de Pagos")
    sys.exit(1)
if not ruta_crp:
    print("Error: No se encontro el Reporte CRP")
    sys.exit(1)
if not ruta_pac:
    print("Error: No se encontro el Reporte PAC")
    sys.exit(1)

ruta_salida = os.path.join(BASE, NOMBRE_SALIDA)
...
print(f"Plantilla Pagos: {len(df_pagos_p)} filas P40 leidas")
...
print(f"Columnas CRP: {df_crp.columns.tolist()}")

# Detectar columnas sin importar mayusculas
col_crp    = next((c for c in df_crp.columns if c.strip().lower() == 'crp'), None)
col_rubro  = next((c for c in df_crp.columns if c.strip().lower() == 'rubro'), None)
col_fondos = next((c for c in df_crp.columns if c.strip().lower() == 'fondos'), None)

if not col_crp:
    col_crp = next((c for c in df_crp.columns if 'crp' in c.strip().lower()), None)
if not col_rubro:
    col_rubro = next((c for c in df_crp.columns if 'rubro' in c.strip().lower()), None)
if not col_fondos:
    col_fondos = next((c for c in df_crp.columns if 'fondo' in c.strip().lower()), None)

print(f"Columna CRP: [{col_crp}] | Rubro: [{col_rubro}] | Fondos: [{col_fondos}]")

if not col_crp or not col_rubro or not col_fondos:
    print("Error: faltan columnas en el Reporte CRP")
    sys.exit(1)

df_crp['crp']    = df_crp[col_crp].astype(str).str.strip()
df_crp['Rubro']  = df_crp[col_rubro].astype(str).str.strip()
df_crp['Fondos'] = df_crp[col_fondos].astype(str).str.strip()
df_crp_uniq = df_crp[['crp', 'Rubro', 'Fondos']].drop_duplicates(subset='crp')
print(f"Reporte CRP: {len(df_crp_uniq)} CRPs unicos")
...
print(f"Reporte PAC: {len(df_pac_data)} filas con disponibilidad")

# ============================================================
# 5. ANALISIS PAC POR RUBRO / FONDO
# ============================================================
alertas = []
    ...
    if pac_mes_actual >= total_pagar:
        ...
        if not otros_meses.empty:
            for _, m in otros_meses.iterrows():
                ...
                detalle += f"      Periodo {m['Per.presup.']} - {nombre_mes:<12}: $ {m['Disponibilidad PAC']:>15,.0f}\n"
            total_otros = otros_meses['Disponibilidad PAC'].sum()
            detalle += f"      TOTAL OTROS MESES: $ {total_otros:>15,.0f}\n"
            if (pac_mes_actual + total_otros) >= total_pagar:
                detalle += f"      Sumando otros meses SI se puede cubrir.\n"
            else:
                detalle += f"      ADVERTENCIA: Incluso sumando todos los meses NO alcanza.\n"
        else:
            detalle += "      NO hay disponibilidad PAC en ningun otro mes.\n"
...
# ============================================================
print("\n" + "="*65)
print("      ANALISIS DISPONIBILIDAD PAC vs PAGOS")
print("="*65)
for a in alertas:
...
filas_resultado.append({'CRP':'','VR A PAGAR':'','RUBRO':'','FONDOS':'','TIPO':'vacio'})
filas_resultado.append({'CRP':'=== ANALISIS DISPONIBILIDAD PAC ===','VR A PAGAR':'','RUBRO':'','FONDOS':'','TIPO':'header_pac'})

for a in alertas:
    filas_resultado.append({
        'CRP': a['estado'], 'VR A PAGAR': '',
        'RUBRO': a['rubro'], 'FONDOS': a['fondo'],
        'TIPO': 'alerta_ok' if 'ALCANZA' == a['estado'] else 'alerta_no'
    })
    mask = (
    ...
    total_pg   = totales_rubro[
        (totales_rubro['Rubro']==a['rubro']) & (totales_rubro['Fondos']==a['fondo'])
    ]['TOTAL_A_PAGAR'].values[0]

    filas_resultado.append({
        'CRP': '  Total a pagar', 'VR A PAGAR': total_pg,
    ...
    if 'NO' in a['estado']:
        ...
        for _, m in otros.iterrows():
            nombre_mes = MESES.get(m['Per.presup.'], m['Per.presup.'])
            filas_resultado.append({
                'CRP': f"  Periodo {m['Per.presup.']} - {nombre_mes}",
...
print(f"__EXCEL__:{NOMBRE_SALIDA}")
